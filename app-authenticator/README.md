...
-   The signature token (JWT) generated by the client in the `x-signature` header with claim `typ` = `app-update-push-id-signature-token`.
...
-   `400 Bad Request` the `x-signature` header has a wrong format or the request body is invalid
-   `401 Unauthorized` The `x-signature` header is missing or verification failed
-   `412 Precondition Failed` The referenced `kid` (authenticator ID) in the signature token does not exist
