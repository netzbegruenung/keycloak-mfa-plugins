...
public class SmsAuthenticatorFactory implements AuthenticatorFactory {
	...
	public List<ProviderConfigProperty> getConfigProperties() {
		return List.of(
			...
			new ProviderConfigProperty("stripFormattingCharacters", "Strip formatting from phone number", "Remove everything but digits and a leading + from phone numbers before sending, e.g. spaces, dashes and parentheses. Applies to every number, including those taken from a user attribute, which never pass through the phone number form. Disable only if your API expects the number verbatim.", ProviderConfigProperty.BOOLEAN_TYPE, true),
